...
export default class News extends Component {
    ...
    async updateNews(){
        const url = `https://newsapi.org/v2/top-headlines?q=${this.props.query}&country=${this.props.country}&&category=${this.props.category}&apiKey=aac6b2707b2845cdb00f990c84930b65&page=${this.state.page}&pageSize=${this.props.pageSize}`;
        ...
        return (
            <div className=''>
                <div style={{margin:"0 auto"}} className="row container">
                    <h2 style={{margin:"30px 0"}} className='text-center my-10'>QuickNews - top {capitalizeFirstLetter(category)} headlines</h2>
                    {loading && <Spinner />}
                ...
                <div className="container d-flex justify-content-between pt-5">
                    ...
                    <button disabled={this.state.page + 1 > Math.ceil(totalResults / pageSize)} type="button" className="btn btn-dark" onClick={this.handleNextClick}>Next &rarr;</button>
                </div>
            </div>
        )
    }
